...
// Impliment an HttpClient
HttpClient client = new HttpClient();
client.BaseAddress = new Uri("https://os.ecgrid.io/v4.1/prod/ECGridOS.asmx/");

IECGridOSClient ecgridClient = new ECGridOSClient(client);
...
string ecgridos_version = ecgridClient.Version().Result;
...
SessionInfo sessionInfo = ecgridClient.WhoAmI(MyAPIKey).Result;
...
ParcelIDInfoCollection parcelPending = ecgridClient.ParcelInBox(MyAPIKey).Result;
...
if (parcelPending != null && parcelPending.TotalRecords > 0)
{
    ...
    foreach (ParcelIDInfo parcel2Download in parcelPending.ParcelIDInfoList)
    {
        ...
        // Download the File
        FileInfo file2Save = ecgridClient.ParcelDownload(MyAPIKey, parcelID).Result;
        if (file2Save != null)
        {
            // Save Base64 String
            ...
            if (File.Exists(fileName))
            {
                ...
                if (ecgridClient.ParcelDownloadConfirm(MyAPIKey, parcelID).Result)
                {
...
// Upload A File
int returnedParcelID = ecgridClient.ParcelUploadA(MyAPIKey, FilenameNoPath, contentsBase64).Result;
